...
\begin{table}[h]
    ...
    \begin{tabular}{|l|l|l|l|}
        ...
        \hline
        Genoa & 2D & Uniform & +12.0\% \\
        \hline
        Genoa & 2D & Exponential & +13.8\% \\
        \hline
        Genoa & 3D & Uniform & +0.8\% \\
        \hline
        Genoa & 3D & Exponential & $-1.2\%$ \\
        \hline
        Icelake & 2D & Uniform & +14.1\% \\
        \hline
        Icelake & 2D & Exponential & +12.9\% \\
        \hline
        Icelake & 3D & Uniform & +2.1\% \\
        \hline
        Icelake & 3D & Exponential & +2.2\%* \\
        \hline
    \end{tabular}
    \caption{Geometric-mean speedup of the cache-aware \texttt{auto} heuristic (at its deployed setting, $\text{CACHE\_FRAC}=0.25$) over the fixed legacy default $S_{\max}=10^5$, for $M=4\times10^8$, averaged over $T\in\{11,13,16,20,25,32,64\}$. The row marked * is computed from a partial subset of thread counts still being completed and will be updated with the full sweep.}
    \label{tab:auto_vs_100k_geomean}
\end{table}

The cache-aware heuristic delivers a consistent improvement in two dimensions on both architectures (12--14\%), and a small but still positive gain in three dimensions on Icelake ($\approx 2\%$). On Genoa, the three-dimensional gain is negligible and mixed in sign, with the per-thread-count improvements roughly cancelling out to a near-zero net effect.
