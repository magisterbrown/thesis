...
These two parameters affect the initial stages of the algorithm before the FFT transform: setting the points and spreading them. The spreading process involves three steps: gathering, where the points are loaded into a thread-local buffer; spreading, where a vectorised kernel is applied to each point and the result is saved to the periodic subgrid; and adding, where the results from the subgrid are added to the global grid in a thread-safe manner.
...
Table~\ref{tab:maxsp_perf_breakdown} shows the resulting slowdown: once the staging buffer
...
Figure~\ref{fig:subgrid_layout} shows where those subgrids actually land. Each bin-row
holds several subproblems that between them tile it along $x$, and one further subproblem
per row boundary that spans the full width because its run of points straddles that
boundary. Everywhere a spanning subgrid underlies a compact one, both write the same
cells, which is the redundant traffic $S_{\max}$ controls. 
