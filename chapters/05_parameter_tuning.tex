...
Across the range of 2D configurations tested -- varying point density, grid aspect ratio, and total problem size, all at a moderate thread count -- the cache-aware auto heuristic typically improves throughput over the legacy fixed default by roughly 4--9\%, as summarised in Table~\ref{tab:maxsp_2d_final_results}. The one exception is the smallest problem size tested (100\,MiB), where auto instead loses $2.0\%$ to the legacy default.

\begin{table}[h]
    \centering
    \begin{tabular}{|l|l|l|r|}
        \hline
        \textbf{Density} & \textbf{$N_1 \times N_2$} & \textbf{Auto $S_{\max}$} & \textbf{Throughput change} \\
        \hline
        0.25 & $11{,}586 \times 11{,}586$ & 2{,}234 & $+4.0\%$ \\
        \hline
        1    & $5{,}700 \times 5{,}700$   & 7{,}409 & $+4.2\%$ \\
        \hline
        1    & $23{,}168 \times 1{,}448$  & 7{,}201 & $+4.1\%$ \\
        \hline
        1    & $22{,}960 \times 1{,}435$  & 7{,}132 & $+5.4\%$ \\
        \hline
        0.5  & $8{,}192 \times 8{,}192$   & 4{,}181 & $+6.0\%$ \\
        \hline
        1    & $1{,}810 \times 1{,}810$   & 8{,}439 & $-2.0\%$ \\
        \hline
        1    & $5{,}793 \times 5{,}793$   & 7{,}134 & $+9.8\%$ \\
        \hline
    \end{tabular}
    \caption{Amortized throughput improvement of the auto heuristic over the legacy fixed default ($S_{\max}=10{,}000$) across 2D type-1 double-precision configurations, all multi-threaded, on the same 16-core machine used for the earlier 2D searches.}
    \label{tab:maxsp_2d_final_results}
\end{table}

The $100\,\text{MiB}$ row's loss is a small-problem effect rather than a cache-fit failure: at this size, the whole transform's nonuniform-point data is already only about twice the CPU's $50\,\text{MiB}$ L3 cache, so there simply is not much total work to spread across subproblems. The auto heuristic's cache-fit computation nonetheless yields a smaller $S_{\max}$ here than the legacy default, which means \emph{more} subproblems than the legacy heuristic would create for the same problem -- and with so little total work to amortise it over, the extra per-subproblem scheduling and synchronisation overhead ends up outweighing whatever cache-locality benefit those smaller subproblems bring.

The same configurations were also re-tested in single precision and, separately, with points drawn from an exponential rather than a uniform distribution (clustered near the origin), to check whether the double-precision, uniform-distribution result above generalises.
...
\begin{table}[h]
    ...
    \begin{tabular}{|l|l|l|r|}
        ...
        \textbf{Density} & \textbf{$N_1 \times N_2$} & \textbf{Auto $S_{\max}$} & \textbf{Throughput change} \\
        \hline
        1 & $4{,}048 \times 4{,}048$   & 8{,}092  & $+2.5\%$ \\
        \hline
        1 & $8{,}096 \times 2{,}024$   & 10{,}067 & $-0.6\%$ \\
        \hline
        1 & $16{,}192 \times 1{,}012$  & 9{,}985  & $-0.3\%$ \\
        \hline
        1 & $32{,}384 \times 506$      & 9{,}984  & $+1.3\%$ \\
        \hline
        1 & $1{,}810 \times 1{,}810$   & 13{,}054 & $+2.4\%$ \\
        \hline
        1 & $5{,}793 \times 5{,}793$   & 9{,}984  & $+0.4\%$ \\
        \hline
    \end{tabular}
    \caption{Same shape/size sweep as Table~\ref{tab:maxsp_2d_final_results}, repeated in single precision.}
    \label{tab:maxsp_2d_float_results}
\end{table}

In single precision the auto heuristic still shows a net improvement, but a smaller one: results range from $-0.6\%$ to $+2.5\%$, compared to the consistent $+4$--$9\%$ win seen in double precision at matching sizes and shapes. This lines up with a direct double-vs-float comparison at the same 1\,GiB configuration, where double gained $+9.8\%$ but float gained only $+0.4\%$: precision changes how much auto helps, though it still helps in both cases.
...
\begin{table}[h]
    ...
    \begin{tabular}{|l|l|l|r|}
        ...
        \textbf{Density} & \textbf{$N_1 \times N_2$} & \textbf{Auto $S_{\max}$} & \textbf{Throughput change} \\
        \hline
        1 & $4{,}048 \times 4{,}048$   & 7{,}409 & $+2.9\%$ \\
        \hline
        1 & $8{,}096 \times 2{,}024$   & 7{,}400 & $+3.6\%$ \\
        \hline
        1 & $16{,}192 \times 1{,}012$  & 7{,}132 & $+5.8\%$ \\
        \hline
        1 & $32{,}384 \times 506$      & 7{,}130 & $+3.6\%$ \\
        \hline
        1 & $1{,}810 \times 1{,}810$   & 8{,}439 & $-2.7\%$ \\
        \hline
        1 & $5{,}793 \times 5{,}793$   & 7{,}134 & $+5.5\%$ \\
        \hline
    \end{tabular}
    \caption{Same shape/size sweep as Table~\ref{tab:maxsp_2d_final_results}, repeated in double precision with points drawn from an exponential distribution instead of uniform.}
    \label{tab:maxsp_2d_exp_results}
\end{table}

Under the exponential distribution the result stays close to the uniform-distribution case: mostly a $+2.9$--$5.8\%$ win, with the same small loss seen under uniform sampling too ($-2.0\%$ there, $-2.7\%$ here). This means that the disproportionately large subgrids produced by the sparse, outlying regions of the exponential distribution (Section~\ref{sec:distributions}) do not end up slowing the transform down: despite their size variance, the auto heuristic performs as well under clustered data as it does under uniformly spread data.
...
\begin{table}[h]
    ...
    \begin{tabular}{|l|l|l|r|r|r|}
        ...
        \hline
        $236 \times 236 \times 236$   & 1    & Single & 64 & 62.49\,M ($S{=}205{,}379$) & $+0.6\%$ \\
        \hline
        $378 \times 378 \times 378$   & 1    & Double & 64 & 22.17\,M ($S{=}843{,}909$) & $+2.3\%$ \\
        \hline
        $378 \times 378 \times 378$   & 1    & Single & 64 & 65.32\,M ($S{=}843{,}909$) & $+0.1\%$ \\
        \hline
...
The average subgrid area $A(S)$ introduced in Section~\ref{sec:cache_aware_subproblem} extends naturally to 1D and 3D by the same saturate-one-axis-then-the-next reasoning. In 1D there is only one axis to saturate, so the subgrid extent simply blends from the bin size $\beta_1$ up to the full grid length $n_1$ as $S$ grows,
\begin{align}
    f &= \min\!\left(1,\; \frac{S}{n_1 \rho}\right), \\
    \text{size}_1 &= n_1 f + (1-f)\beta_1 + w, \\
    A(S) &= \text{size}_1 \cdot b + 16S + 8dS .
    \label{eq:subgrid_area_1d}
\end{align}
In 3D, the same pattern is applied across all three axes in turn: the first axis saturates against a thin $\beta_2 \times \beta_3$ cross-section, the second axis then saturates against a full-width, $\beta_3$-thick slab, and the third axis simply stacks additional whole $\beta_3$-thick slabs without bound once the first two are full,
\begin{align}
    ...
    A(S) &= \text{size}_1 \cdot \text{size}_2 \cdot \text{size}_3 \cdot b + 16S + 8dS .
    \label{eq:subgrid_area_3d}
\end{align}
In both cases, as in the 2D case above, the final $16S + 8dS$ term folds the point buffer's own byte footprint into $A(S)$ alongside the subgrid itself.
