...
Figure~\ref{fig:maxsp_steps} measures the cost of a subproblem across the range, as wall-clock time, on a 2D transform of $1$\,GiB of nonuniform points at unit density, on all threads of the test platform of Section~\ref{sec:platform}. The horizontal axis is $S_{\max}$ on a logarithmic scale, from $10^4$ up to $M/n_{\mathrm{thr}} = 524{,}288$, the value at which each thread receives a single subproblem and beyond which the spreader ignores further increases; the number of subproblems the transform is divided into therefore falls from $3355$ at the left edge to $40$ at the right. The curve is the time the spreading phase takes, that is gathering, spreading and adding together, as measured by FINUFFT's own timer.

The steps themselves are compared in cycles rather than wall-clock time, in Figure~\ref{fig:maxsp_cycles}, because cycles count work actually performed and summed over threads, whereas the wall time of a phase also counts threads that have run out of work and are waiting. Measured that way the two large steps move against each other. Adding falls from $10.0$ to $1.8$ billion cycles, a factor of $5.5$, because fewer and larger subproblems produce less total subgrid volume to write back: the padding of half a kernel width around every subgrid is paid once per subproblem, so its share of the whole shrinks as subproblems grow. Gathering moves the other way, from $7.4$ to $12.2$ billion cycles, but only by a factor of $1.7$, and it stops growing beyond $S_{\max} \approx 10^5$, peaking at $13.3$ billion near $1.6 \times 10^5$. Spreading is the least sensitive of the three, drifting down by about a quarter, from $4.3$ to $3.2$ billion cycles.

In wall-clock time the picture is different, and Figure~\ref{fig:maxsp_steps} shows why the choice of $S_{\max}$ still matters. The total time of a subproblem is flat at about $112$\,ms over the first three points, where the falling and rising terms cancel almost exactly, and then climbs steadily to $207$\,ms at the top of the range. The best setting for this transform is therefore at the small end of the range, and the penalty for choosing the largest permissible value is a factor of $1.8$.

The gap between the two views is load imbalance. The three steps together account for about $22$ billion cycles at the bottom of the range and $17$ billion at the top, so the work done actually falls slightly, while the wall time nearly doubles. At the largest subproblem sizes there is barely one subproblem per thread, so the phase ends only when the slowest thread finishes and the rest stand idle; that idle time lengthens the phase without being charged to any step.
...
The current heuristic sets $S_{\max}$ to the minimum of $10^5$ and $N/n_{\text{thr}}$ for 2D and 3D transforms, and to the minimum of $10^4$ and $N/n_{\text{thr}}$ for 1D transforms. These constants are suboptimal: they introduce extra OpenMP scheduling overhead, and for transforms with very low density, the subgrid can end up very large and spill out of cache into main memory.
